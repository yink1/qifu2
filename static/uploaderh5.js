function ksort(inputArr, sort_flags) {
    ...
    for (keys.sort(sorter), this.php_js = this.php_js || {},
    this.php_js.ini = this.php_js.ini || {},
    strictForIn = this.php_js.ini["phpjs.strictForIn"] && this.php_js.ini["phpjs.strictForIn"].local_value && "off" !== this.php_js.ini["phpjs.strictForIn"].local_value, populateArr = strictForIn ? inputArr: populateArr, i = 0; i < keys.length; i++) k = keys[i],
    tmp_arr[k] = inputArr[k],
    strictForIn && delete inputArr[k];
...
function(global) {
    function getMappingArgs(fn) {
        ...
        i = 0;
        for (args[0] || (args = []); args[i];) args[i] = require(args[i]),
        i += 1;
        return args
    }
    function newInst(key, ifExist) { (ifExist ? ns.instances[key] : !ns.instances[key]) && ns.modules[key] && (ns.instances[key] = ns.modules[key].apply(window, getMappingArgs(ns.modules[key])))
    }
    ...
    function core(key, target) {
        if (ns.modules[key] = target, newInst(key, !0), waiter[key]) {
            for (var i = 0; waiter[key][i];) waiter[key][i](require(key)),
            i += 1;
            delete waiter[key]
        }
    }
...
function(h, r) {
    ...
    s = m.Hex = {
        ...
        parse: function(a) {
            for (var b = a.length,
            d = [], c = 0; b > c; c += 2) d[c >>> 3] |= parseInt(a.substr(c, 2), 16) << 24 - 4 * (c % 8);
    ...
    p = m.Latin1 = {
        ...
        parse: function(a) {
            for (var b = a.length,
            d = [], c = 0; b > c; c++) d[c >>> 2] |= (255 & a.charCodeAt(c)) << 24 - 8 * (c % 4);
...
function(g, l) {
    ...
    n = b.Hex = {
        ...
        parse: function(a) {
            for (var c = a.length,
            b = [], f = 0; c > f; f += 2) b[f >>> 3] |= parseInt(a.substr(f, 2), 16) << 24 - 4 * (f % 8);
    ...
    j = b.Latin1 = {
        ...
        parse: function(a) {
            for (var c = a.length,
            b = [], f = 0; c > f; f++) b[f >>> 2] |= (255 & a.charCodeAt(f)) << 24 - 8 * (f % 4);
...
function() {
    ...
    return g_win.jQuery ? g_win.jQuery: (function(e, t) {
        ...
        function Kn(e, t, n) {
            ...
            f.progressTC(f.opts.progressTC).done(f.opts.done, f.opts.complete).fail(f.opts.fail).always(f.opts.always)
        }
        function Qn(e, t) {
            var n, r, i, s, o;
        ...
        A = function() {
            i.addEventListener ? (i.removeEventListener("DOMContentLoadedTC", A, !1), v.ready()) : "complete" === i.readyState && (i.detachEvent("onreadystatechange", A), v.ready())
        },
        O = {};
        v.fn = v.prototype = {
            constructor: v,
        ...
        v.ready.promise = function(t) {
            ...
            else if (i.addEventListener) i.addEventListener("DOMContentLoadedTC", A, !1),
            e.addEventListener("loadTC", v.ready, !1);
            else {
        ...
        v.extend({
            Deferred: function(e) {
                var t = [["resolve", "done", v.Callbacks("once memory"), "resolved"], ["reject", "fail", v.Callbacks("once memory"), "rejected"], ["notify", "progressTC", v.Callbacks("memory")]],
                n = "pending",
                r = {
                    state: function() {
                        return n
                        ...
                        return v.Deferred(function(n) {
                            ...
                            function(t, r) {
                                ...
                                function() {
                                    ...
                                    e && v.isFunction(e.promise) ? e.promise().done(n.resolve).fail(n.reject).progressTC(n.notify) : n[s + "With"](this === i ? n: this, [e])
                                }: n[s])
                            }),
                            e = null
                        }).promise()
            ...
            when: function(e) {
                ...
                if (r > 1) for (u = new Array(r), a = new Array(r), f = new Array(r); r > t; t++) n[t] && v.isFunction(n[t].promise) ? n[t].promise().done(o(t, f, n)).fail(s.reject).progressTC(o(t, a, u)) : --i;
        ...
        v.each("blur focus focusin focusout loadTC resize scroll unload click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error contextmenu".split(" "),
        function(e, t) {
            v.fn[t] = function(e, n) {
        ...
        var ct = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progressTC|section|summary|time|video",
...
function(factory) {
    ...
    function concatenateArrayBuffers(first, second, returnUInt8Array) {
        ...
        return result.set(new Uint8Array(first)),
    ...
    function() {
        function clamp(val, length) {
            return val = 0 | val || 0,
            0 > val ? Math.max(val + length, 0) : Math.min(val, length)
        }
        ArrayBuffer.prototype.slice = function(from, to) {
            var num, target, targetArray, sourceArray, length = this.byteLength,
            begin = clamp(from, length),
            end = length;
            return to !== undefined && (end = clamp(to, length)),
            begin > end ? new ArrayBuffer(0) : (num = end - begin, target = new ArrayBuffer(num), targetArray = new Uint8Array(target), sourceArray = new Uint8Array(this, begin, num), targetArray.set(sourceArray), target)
        }
    } (),
    SparkMD5.prototype.append = function(str) {
        return this.appendBinary(toUtf8(str)),
        this
    },
    ...
    SparkMD5.prototype.end = function(raw) {
        ...
        return this._finish(tail, length),
        ret = hex(this._hash),
    ...
    SparkMD5.ArrayBuffer.prototype.append = function(arr) {
        ...
        return this._buff = length > i - 64 ? new Uint8Array(buff.buffer.slice(i - 64)) : new Uint8Array(0),
        this
    },
    ...
    SparkMD5.ArrayBuffer.prototype.getState = function() {
        ...
        return state.buff = arrayBuffer2Utf8Str(state.buff),
        state
    },
    SparkMD5.ArrayBuffer.prototype.setState = function(state) {
        return state.buff = utf8Str2ArrayBuffer(state.buff, !0),
...
function(exports, undefined) {
    ...
    function resolve(id) {
        for (var target = exports,
        fragments = id.split(/[.\/]/), fi = 0; fi < fragments.length; ++fi) {
    ...
    function expose(ids) {
        for (var i = 0; i < ids.length; i++) {
            for (var target = exports,
            id = ids[i], fragments = id.split(/[.\/]/), fi = 0; fi < fragments.length - 1; ++fi) target[fragments[fi]] === undefined && (target[fragments[fi]] = {}),
            target = target[fragments[fi]];
            target[fragments[fragments.length - 1]] = modules[id]
        }
    }
    var modules = {};
    define("moxie/core/utils/Basic", [],
    function() {
        ...
        inSeries = function(queue, cb) {
            function callNext(i) {
                "function" === typeOf(queue[i]) && queue[i](function(error) {++i < length && !error ? callNext(i) : cb(error)
                })
            }
            var i = 0,
            length = queue.length;
        ...
        inParallel = function(queue, cb) {
            ...
            function(fn, i) {
                fn(function(error) {
                    ...
                    cbArgs[i] = args,
                    count++,
                    count === num && (cbArgs.unshift(null), cb.apply(this, cbArgs))
                })
            })
        },
        inArray = function(needle, array) {
            if (array) {
                ...
                for (var i = 0,
                length = array.length; length > i; i++) if (array[i] === needle) return i
            }
            return - 1
        },
        arrayDiff = function(needles, array) {
            var diff = [];
            "array" !== typeOf(needles) && (needles = [needles]),
            "array" !== typeOf(array) && (array = [array]);
        ...
        arrayIntersect = function(array1, array2) {
            var result = [];
            return each(array1,
            function(item) { - 1 !== inArray(item, array2) && result.push(item)
            }),
        ...
        parseSizeStr = function(size) {
            ...
            return size = /^([0-9\.]+)([tmgk]?)$/.exec(size.toLowerCase().replace(/[^0-9\.tmkg]/g, "")),
            mul = size[2],
            size = +size[1],
            muls.hasOwnProperty(mul) && (size *= muls[mul]),
            Math.floor(size)
        },
    ...
    define("moxie/core/utils/Env", ["moxie/core/utils/Basic"],
    function(Basic) {
        ...
        can = function() {
            var caps = {
                ...
                return_response_type: function(responseType) {
                    try {
                        ...
                        if (window.XMLHttpRequest) {
                            ...
                            if (xhr.open("get", "/"), "responseType" in xhr) return xhr.responseType = responseType,
                            xhr.responseType !== responseType ? !1 : !0
                        }
                    } catch(ex) {}
                    return ! 1
                },
                ...
                use_fileinput: function() {
                    ...
                    return el.setAttribute("type", "file"),
                    !el.disabled
                }
            };
            return function(cap) {
                var args = [].slice.call(arguments);
                return args.shift(),
                "function" === Basic.typeOf(caps[cap]) ? caps[cap].apply(this, args) : !!caps[cap]
            }
        } (),
        ...
        Env.log = function() {
            ...
            if ("string" === Basic.typeOf(data) && (data = Basic.sprintf.apply(this, arguments)), window && window.console && window.console.log) window.console.log(data);
                ...
                console || (console = document.createElement("pre"), console.id = "moxie-console", document.body.appendChild(console)),
                -1 !== Basic.inArray(Basic.typeOf(data), ["object", "array"]) ? logObj(data) : console.appendChild(document.createTextNode(data + "\n"))
            }
        }),
        Env
    }),
    define("moxie/core/I18n", ["moxie/core/utils/Basic"],
    function(Basic) {
        var i18n = {};
        return {
    ...
    define("moxie/core/utils/Mime", ["moxie/core/utils/Basic", "moxie/core/I18n"],
    function(Basic, I18n) {
        ...
        Mime = {
            ...
            mimes2exts: function(mimes) {
                var self = this,
                exts = [];
                return Basic.each(mimes,
                function(mime) {
                    if ("*" === mime) return exts = [],
                    !1;
                    var m = mime.match(/^(\w+)\/(\*|\w+)$/);
                    m && ("*" === m[2] ? Basic.each(self.extensions,
                    function(arr, mime) {
            ...
            mimes2extList: function(mimes) {
                var accept = [],
                exts = [];
                return "string" === Basic.typeOf(mimes) && (mimes = Basic.trim(mimes).split(/\s*,\s*/)),
        ...
        return Mime.addMimeType(mimeData),
        Mime
    }),
    define("moxie/core/utils/Dom", ["moxie/core/utils/Env"],
    function(Env) {
        var get = function(id) {
        ...
        getPos = function(node, root) {
            ...
            y = 0,
            doc = document;
            if (node = node, root = root || doc.body, node && node.getBoundingClientRect && "IE" === Env.browser && (!doc.documentMode || doc.documentMode < 8)) return nodeRect = getIEPos(node),
            rootRect = getIEPos(root),
            {
                x: nodeRect.x - rootRect.x,
                y: nodeRect.y - rootRect.y
            };
            for (parent = node; parent && parent != root && parent.nodeType;) x += parent.offsetLeft || 0,
            y += parent.offsetTop || 0,
            parent = parent.offsetParent;
            for (parent = node.parentNode; parent && parent != root && parent.nodeType;) x -= parent.scrollLeft || 0,
    ...
    define("moxie/core/Exceptions", ["moxie/core/utils/Basic"],
    function(Basic) {
        ...
        return {
            RuntimeError: function() {
                function RuntimeError(code) {
                    ...
                    this.messageTC = this.name + ": RuntimeError " + this.code
                }
                var namecodes = {
                    NOT_INIT_ERR: 1,
                    NOT_SUPPORTED_ERR: 9,
                ...
                return Basic.extend(RuntimeError, namecodes),
            ...
            ImageError: function() {
                function ImageError(code) {
                    this.code = code,
                    this.name = _findKey(namecodes, code),
                    this.messageTC = this.name + ": ImageError " + this.code
                }
                var namecodes = {
                    WRONG_FORMAT: 1,
                ...
                return Basic.extend(ImageError, namecodes),
            ...
            FileException: function() {
                function FileException(code) {
                    this.code = code,
                    this.name = _findKey(namecodes, code),
                    this.messageTC = this.name + ": FileException " + this.code
                }
                var namecodes = {
                    NOT_FOUND_ERR: 1,
                    SECURITY_ERR: 2,
                ...
                return Basic.extend(FileException, namecodes),
            ...
            DOMException: function() {
                function DOMException(code) {
                    this.code = code,
                    this.name = _findKey(namecodes, code),
                    this.messageTC = this.name + ": DOMException " + this.code
                }
                var namecodes = {
                    INDEX_SIZE_ERR: 1,
                ...
                return Basic.extend(DOMException, namecodes),
    ...
    define("moxie/core/EventTarget", ["moxie/core/utils/Env", "moxie/core/Exceptions", "moxie/core/utils/Basic"],
        ...
        function EventTarget() {
            ...
            Basic.extend(this, {
                ...
                addEventListener: function(type, fn, priority, scope) {
                    var list, self = this;
                    return this.hasOwnProperty("uid") || (this.uid = Basic.guid("uid_")),
                    type = Basic.trim(type),
                    /\s/.test(type) ? void Basic.each(type.split(/\s+/),
                    function(type) {
                ...
                dispatchEvent: function(type) {
                    ...
                    if ("string" !== Basic.typeOf(type)) {
                        ...
                        type = tmpEvt.type,
                        tmpEvt.total !== undef && tmpEvt.loaded !== undef && (evt.total = tmpEvt.total, evt.loaded = tmpEvt.loaded),
                        evt.async = tmpEvt.async || !1
                    }
                    if ( - 1 !== type.indexOf("::") ? !
                    function(arr) {
                        uid = arr[0],
                        ...
                        evt.type = type,
                        args.unshift(evt),
                        MXI_DEBUG && Env.debug.events && Env.log("Event '%s' fired on %u", evt.type, uid);
                        var queue = [];
                        Basic.each(list,
                        function(handler) {
                            args[0].target = handler.scope,
                            evt.async ? queue.push(function(cb) {
                                setTimeout(function() {
                        ...
                        queue.length && Basic.inSeries(queue,
                        function(err) {
                            result = !err
                        })
                    }
                ...
                handleEventProps: function(dispatches) {
                    var self = this;
                    this.bind(dispatches.join(" "),
                    function(e) {
                        var prop = "on" + e.type.toLowerCase();
                        "function" === Basic.typeOf(this[prop]) && this[prop].apply(this, arguments)
                    }),
                    Basic.each(dispatches,
                    function(prop) {
                        prop = "on" + prop.toLowerCase(prop),
                        "undefined" === Basic.typeOf(self[prop]) && (self[prop] = null)
                    })
                }
            })
        }
        return EventTarget.instance = new EventTarget,
        EventTarget
    }),
    define("moxie/runtime/Runtime", ["moxie/core/utils/Env", "moxie/core/utils/Basic", "moxie/core/utils/Dom", "moxie/core/EventTarget"],
    function(Env, Basic, Dom, EventTarget) {
        function Runtime(options, type, caps, modeCaps, preferredMode) {
            ...
            caps),
            options.preferred_caps && (defaultMode = Runtime.getMode(modeCaps, options.preferred_caps, defaultMode)),
            MXI_DEBUG && Env.debug.runtime && Env.log("	default mode: %s", defaultMode),
            _shim = function() {
                var objpool = {};
                return {
                    ...
                    removeAllInstances: function() {
                        ...
                        Basic.each(objpool,
                        function(obj, uid) {
                            "function" === Basic.typeOf(obj.instance.destroy) && obj.instance.destroy.call(obj.context),
            ...
            Basic.extend(this, {
                ...
                getShimContainer: function() {
                    ...
                    return shimContainer || (container = this.options.container ? Dom.get(this.options.container) : document.body, shimContainer = document.createElement("div"), shimContainer.id = this.shimid, shimContainer.className = "moxie-shim moxie-shim-" + this.type, Basic.extend(shimContainer.style, {
                        ...
                        height: "1px",
                        overflow: "hidden"
                    }), container.appendChild(shimContainer), container = null),
                    shimContainer
                },
                getShim: function() {
                    return _shim
                ...
                destroy: function() {
                    if (self) {
                        ...
                        shimContainer && shimContainer.parentNode.removeChild(shimContainer),
                        _shim && _shim.removeAllInstances(),
                        this.unbindAll(),
        ...
        runtimes = {};
        return Runtime.order = "html5,flash,silverlight,html4",
        ...
        Runtime.parseCaps = function(capStr) {
            var capObj = {};
            return "string" !== Basic.typeOf(capStr) ? capStr || {}: (Basic.each(capStr.split(","),
            function(key) {
                capObj[key] = !0
            }), capObj)
        },
        ...
        Runtime.getMode = function(modeCaps, requiredCaps, defaultMode) {
            ...
            if ("undefined" === Basic.typeOf(defaultMode) && (defaultMode = "browser"), requiredCaps && !Basic.isEmptyObj(modeCaps)) {
                if (Basic.each(requiredCaps,
                function(value, cap) {
                    if (modeCaps.hasOwnProperty(cap)) {
                        var capMode = modeCaps[cap](value);
                        if ("string" == typeof capMode && (capMode = [capMode]), mode) {
                            if (! (mode = Basic.arrayIntersect(mode, capMode))) return MXI_DEBUG && Env.debug.runtime && Env.log("		%c: %v (conflicting mode requested: %s)", cap, value, capMode),
                            mode = !1
                        } else mode = capMode
                    }
    ...
    define("moxie/runtime/RuntimeClient", ["moxie/core/utils/Env", "moxie/core/Exceptions", "moxie/core/utils/Basic", "moxie/runtime/Runtime"],
            ...
            Basic.extend(this, {
                connectRuntime: function(options) {
                    function initialize(items) {
                        ...
                        return items.length ? (type = items.shift().toLowerCase(), (constructor = Runtime.getConstructor(type)) ? (MXI_DEBUG && Env.debug.runtime && (Env.log("Trying runtime: %s", type), Env.log(options)), runtime = new constructor(options), runtime.bind("Init",
                        function() {
                            runtime.initialized = !0,
                            MXI_DEBUG && Env.debug.runtime && Env.log("Runtime '%s' initialized", runtime.type),
                            setTimeout(function() {
                                runtime.clients++,
                        ...
                        function() {
                            MXI_DEBUG && Env.debug.runtime && Env.log("Runtime '%s' failed to initialize", runtime.type),
                            runtime.destroy(),
                            initialize(items)
                    ...
                    if ("string" === Basic.typeOf(options) ? ruid = options: "string" === Basic.typeOf(options.ruid) && (ruid = options.ruid), ruid) {
                        if (runtime = Runtime.getRuntime(ruid)) return runtime.clients++,
                        runtime;
                ...
                disconnectRuntime: function() {
                    runtime && --runtime.clients <= 0 && runtime.destroy(),
                    runtime = null
                },
                getRuntime: function() {
    ...
    define("moxie/file/FileInput", ["moxie/core/utils/Basic", "moxie/core/utils/Env", "moxie/core/utils/Mime", "moxie/core/utils/Dom", "moxie/core/Exceptions", "moxie/core/EventTarget", "moxie/core/I18n", "moxie/runtime/Runtime", "moxie/runtime/RuntimeClient"],
    function(Basic, Env, Mime, Dom, x, EventTarget, I18n, Runtime, RuntimeClient) {
        function FileInput(options) {
            ...
            defaults, options),
            "string" == typeof options.required_caps && (options.required_caps = Runtime.parseCaps(options.required_caps)),
            "string" == typeof options.accept && (options.accept = Mime.mimes2extList(options.accept)),
            container = Dom.get(options.container),
            container || (container = document.body),
            "static" === Dom.getStyle(container, "position") && (container.style.position = "relative"),
            ...
            Basic.extend(self, {
                ...
                init: function() {
                    ...
                    function(e, runtime) {
                        ...
                        function() {
                            ...
                            browseButton && (pos = Dom.getPos(browseButton, Dom.get(options.container)), size = Dom.getSize(browseButton), shimContainer && Basic.extend(shimContainer.style, {
                                top: pos.y + "px",
                                left: pos.x + "px",
                                width: size.w + "px",
                                height: size.h + "px"
                    ...
                    self.connectRuntime(Basic.extend({},
                    options, {
                        required_caps: {
                            select_file: !0
                        }
                ...
                destroy: function() {
                    var runtime = this.getRuntime();
                    runtime && (runtime.exec.call(this, "FileInput", "destroy"), this.disconnectRuntime()),
                    "array" === Basic.typeOf(this.files) && Basic.each(this.files,
                    function(file) {
                        file.destroy()
                    }),
                    this.files = null,
        ...
        return FileInput.prototype = EventTarget.instance,
        FileInput
    }),
    define("moxie/core/utils/Encode", [],
    function() {
    ...
    define("moxie/file/Blob", ["moxie/core/utils/Basic", "moxie/core/utils/Encode", "moxie/runtime/RuntimeClient"],
    function(Basic, Encode, RuntimeClient) {
        function Blob(ruid, blob) {
            ...
            blob ? "string" === Basic.typeOf(blob) && (blob = {
                data: blob
            }) : blob = {},
            Basic.extend(this, {
    ...
    define("moxie/file/File", ["moxie/core/utils/Basic", "moxie/core/utils/Mime", "moxie/file/Blob"],
    function(Basic, Mime, Blob) {
        function File(ruid, file) {
            file || (file = {}),
            Blob.apply(this, arguments),
            this.type || (this.type = Mime.getFileMime(file.name));
            var name;
            if (file.name) name = file.name.replace(/\\/g, "/"),
            name = name.substr(name.lastIndexOf("/") + 1);
            else if (this.type) {
                var prefix = this.type.split("/")[0];
                name = Basic.guid(("" !== prefix ? prefix: "file") + "_"),
                Mime.extensions[this.type] && (name += "." + Mime.extensions[this.type][0])
            }
            Basic.extend(this, {
        ...
        return File.prototype = Blob.prototype,
        File
    }),
    define("moxie/file/FileDrop", ["moxie/core/I18n", "moxie/core/utils/Dom", "moxie/core/Exceptions", "moxie/core/utils/Basic", "moxie/core/utils/Env", "moxie/file/File", "moxie/runtime/RuntimeClient", "moxie/core/EventTarget", "moxie/core/utils/Mime"],
    function(I18n, Dom, x, Basic, Env, File, RuntimeClient, EventTarget, Mime) {
        function FileDrop(options) {
            ...
            "static" === Dom.getStyle(options.container, "position") && (options.container.style.position = "relative"),
            "string" == typeof options.accept && (options.accept = Mime.mimes2extList(options.accept)),
            RuntimeClient.call(self),
            Basic.extend(self, {
                uid: Basic.guid("uid_"),
                ...
                destroy: function() {
                    var runtime = this.getRuntime();
                    runtime && (runtime.exec.call(this, "FileDrop", "destroy"), this.disconnectRuntime()),
                    this.files = null,
                    this.unbindAll()
                }
            }),
        ...
        return FileDrop.prototype = EventTarget.instance,
        FileDrop
    }),
    define("moxie/file/FileReader", ["moxie/core/utils/Basic", "moxie/core/utils/Encode", "moxie/core/Exceptions", "moxie/core/EventTarget", "moxie/file/Blob", "moxie/runtime/RuntimeClient"],
    function(Basic, Encode, x, EventTarget, Blob, RuntimeClient) {
        function FileReader() {
            function _read(op, blob) {
                if (this.trigger("loadstart"), this.readyState === FileReader.LOADING) return this.trigger("error", new x.DOMException(x.DOMException.INVALID_STATE_ERR)),
                void this.trigger("loadendTC");
                if (! (blob instanceof Blob)) return this.trigger("error", new x.DOMException(x.DOMException.NOT_FOUND_ERR)),
                void this.trigger("loadendTC");
                if (this.result = null, this.readyState = FileReader.LOADING, blob.isDetached()) {
                    ...
                    this.trigger("loadTC"),
                    this.trigger("loadendTC")
                } else this.connectRuntime(blob.ruid),
            ...
            Basic.extend(this, {
                ...
                abort: function() {
                    this.result = null,
                    -1 === Basic.inArray(this.readyState, [FileReader.EMPTY, FileReader.DONE]) && (this.readyState === FileReader.LOADING && (this.readyState = FileReader.DONE), this.exec("FileReader", "abort"), this.trigger("abort"), this.trigger("loadendTC"))
                },
                destroy: function() {
                    this.abort(),
        ...
        var dispatches = ["loadstart", "progressTC", "loadTC", "abort", "error", "loadendTC"];
        return FileReader.EMPTY = 0,
    ...
    function() {
        var parseUrl = function(url, currentUrl) {
            ...
            uri = {},
            regex = /^(?:([^:\/?#]+):)?(?:\/\/()(?:(?:()(?:([^:@\/]*):?([^:@\/]*))?@)?([^:\/?#]*)(?::(\d*))?))?()(?:(()(?:(?:[^?#\/]*\/)*)()(?:[^?#]*))(?:\\?([^#]*))?(?:#(.*))?)/, m = regex.exec(url || ""); i--;) m[i] && (uri[key[i]] = m[i]);
            if (!uri.scheme) {
                currentUrl && "string" != typeof currentUrl || (currentUrl = parseUrl(currentUrl || document.location.href)),
                uri.scheme = currentUrl.scheme,
                uri.host = currentUrl.host,
                uri.port = currentUrl.port;
                var path = "";
                /^[^\/]/.test(uri.path) && (path = currentUrl.path, path = /\/[^\/]*\.[^\/]*$/.test(path) ? path.replace(/\/[^\/]+$/, "/") : path.replace(/\/?$/, "/")),
                uri.path = path + (uri.path || "")
            }
            return uri.port || (uri.port = ports[uri.scheme] || 80),
        ...
        hasSameOrigin = function(url) {
            ...
            return "string" == typeof url && (url = parseUrl(url)),
    ...
    define("moxie/runtime/RuntimeTarget", ["moxie/core/utils/Basic", "moxie/runtime/RuntimeClient", "moxie/core/EventTarget"],
    function(Basic, RuntimeClient, EventTarget) {
        ...
        return RuntimeTarget.prototype = EventTarget.instance,
        RuntimeTarget
    }),
    define("moxie/file/FileReaderSync", ["moxie/core/utils/Basic", "moxie/runtime/RuntimeClient", "moxie/core/utils/Encode"],
        ...
        return function() {
            function _read(op, blob) {
                ...
                case "readAsText":
                    for (var txt = "",
                    i = 0,
                    length = src.length; length > i; i++) txt += String.fromCharCode(src[i]);
                    return txt
                }
            }
            RuntimeClient.call(this),
    ...
    define("moxie/xhr/FormData", ["moxie/core/Exceptions", "moxie/core/utils/Basic", "moxie/file/Blob"],
            ...
            Basic.extend(this, {
                append: function(name, value) {
                    ...
                    value instanceof Blob ? _blob = {
                        name: name,
                        value: value
                    }: "array" === valueType ? (name += "[]", Basic.each(value,
                    function(value) {
                        self.append(name, value)
                    })) : "object" === valueType ? Basic.each(value,
                    function(value, key) {
    ...
    define("moxie/xhr/XMLHttpRequest", ["moxie/core/utils/Basic", "moxie/core/Exceptions", "moxie/core/EventTarget", "moxie/core/utils/Encode", "moxie/core/utils/Url", "moxie/runtime/Runtime", "moxie/runtime/RuntimeTarget", "moxie/file/Blob", "moxie/file/FileReaderSync", "moxie/xhr/FormData", "moxie/core/utils/Env", "moxie/core/utils/Mime"],
    function(Basic, x, EventTarget, Encode, Url, Runtime, RuntimeTarget, Blob, FileReaderSync, FormData, Env, Mime) {
        ...
        function XMLHttpRequest() {
            ...
            function _doXHR(data) {
                function loadendTC() {
                    _xhr && (_xhr.destroy(), _xhr = null),
                    self.dispatchEvent("loadendTC"),
                    self = null
                }
                function exec(runtime) {
                    ...
                    function(e) {
                        ...
                        _upload_events_flag && self.upload.dispatchEvent(e)
                    }),
                    _xhr.bind("progressTC",
                    function(e) {
                        _p("readyState") !== XMLHttpRequest.LOADING && (_p("readyState", XMLHttpRequest.LOADING), self.dispatchEvent("readystatechange")),
                        self.dispatchEvent(e)
                    }),
                    _xhr.bind("UploadProgress",
                    function(e) {
                    ...
                    function(e) {
                        ...
                        ~Basic.inArray(_p("responseType"), ["text", ""]) ? _p("responseText", _p("response")) : "document" === _p("responseType") && _p("responseXML", _p("response")),
                        _responseHeaders = runtime.exec.call(_xhr, "XMLHttpRequest", "getAllResponseHeaders"),
                        self.dispatchEvent("readystatechange"),
                        _p("status") > 0 ? (_upload_events_flag && self.upload.dispatchEvent(e), self.dispatchEvent(e)) : (_error_flag = !0, self.dispatchEvent("error")),
                        loadendTC()
                    }),
                    _xhr.bind("Abort",
                    function(e) {
                ...
                "string" == typeof _options.required_caps && (_options.required_caps = Runtime.parseCaps(_options.required_caps)),
                _options.required_caps = Basic.extend({},
                _options.required_caps, {
                    return_response_type: self.responseType
                }),
                data instanceof FormData && (_options.required_caps.send_multipart = !0),
                Basic.isEmptyObj(_headers) || (_options.required_caps.send_custom_headers = !0),
                _same_origin_flag || (_options.required_caps.do_cors = !0),
                _options.ruid ? exec(_xhr.connectRuntime(_options)) : (_xhr.bind("RuntimeInit",
                function(e, runtime) {
                    exec(runtime)
                }), _xhr.bind("RuntimeError",
                function(e, err) {
            ...
            Basic.extend(this, props, {
                ...
                setRequestHeader: function(header, value) {
                    ...
                    return header = Basic.trim(header).toLowerCase(),
                    ~Basic.inArray(header, uaHeaders) || /^(proxy\-|sec\-)/.test(header) ? !1 : (_headers[header] ? _headers[header] += ", " + value: _headers[header] = value, !0)
                },
                getAllResponseHeaders: function() {
                ...
                getResponseHeader: function(header) {
                    ...
                    _error_flag || ~Basic.inArray(header, ["set-cookie", "set-cookie2"]) ? null: _responseHeaders && "" !== _responseHeaders && (_responseHeadersBag || (_responseHeadersBag = {},
                ...
                send: function(data, options) {
                    ...
                    this.readyState !== XMLHttpRequest.OPENED || _send_flag) throw new x.DOMException(x.DOMException.INVALID_STATE_ERR);
                    if (data instanceof Blob) _options.ruid = data.ruid,
                    ...
                    this.withCredentials || (this.withCredentials = _options.required_caps && _options.required_caps.send_browser_cookies && !_same_origin_flag),
                    _upload_events_flag = !_sync_flag && this.upload.hasEventListener(),
                    _error_flag = !1,
                    _upload_complete_flag = !data,
                    _sync_flag || (_send_flag = !0),
                ...
                destroy: function() {
                    _xhr && ("function" === Basic.typeOf(_xhr.destroy) && _xhr.destroy(), _xhr = null),
                    this.unbindAll(),
        ...
        var dispatches = ["loadstart", "progressTC", "abort", "error", "loadTC", "timeout", "loadendTC"];
        return XMLHttpRequest.UNSENT = 0,
    ...
    define("moxie/runtime/Transporter", ["moxie/core/utils/Basic", "moxie/core/utils/Encode", "moxie/runtime/RuntimeClient", "moxie/core/EventTarget"],
    function(Basic, Encode, RuntimeClient, EventTarget) {
        function Transporter() {
            ...
            function _run(type, runtime) {
                var self = this;
                _runtime = runtime,
                self.bind("TransportingProgress",
                function(e) {
                    _pos = e.loaded,
                    _size > _pos && -1 === Basic.inArray(self.state, [Transporter.IDLE, Transporter.DONE]) && _transport.call(self)
                },
                999),
                self.bind("TransportingComplete",
                function() {
                    _pos = _size,
            ...
            function _transport() {
                ...
                _chunk_size > bytesLeft && (_chunk_size = bytesLeft),
            ...
            Basic.extend(this, {
                ...
                abort: function() {
                    ...
                    _runtime && (_runtime.exec.call(self, "Transporter", "clear"), self.trigger("TransportingAborted")),
                    _reset.call(self)
                },
                destroy: function() {
                    this.unbindAll(),
    ...
    function(Basic, Dom, x, FileReaderSync, XMLHttpRequest, Runtime, RuntimeClient, Transporter, Env, EventTarget, Blob, File, Encode) {
        function Image() {
            function _updateInfo(info) {
                info || (info = this.exec("Image", "getInfo")),
                this.size = info.size,
            ...
            function _loadFromImage(img, exact) {
                ...
                runtime.exec.call(this, "Image", "loadFromImage", img, "undefined" === Basic.typeOf(exact) ? !0 : exact)
            }
            function _loadFromBlob(blob, options) {
                ...
                var self = this;
                self.name = blob.name || "",
                blob.isDetached() ? (this.bind("RuntimeInit",
                function(e, runtime) {
                    exec(runtime)
            ...
            Basic.extend(this, {
                ...
                meta: {},
                clone: function() {
                    this.loadTC.apply(this, arguments)
                },
                loadTC: function() {
                ...
                embed: function(el, opts) {
                    function render(type, quality) {
                        ...
                        if (Env.can("create_canvas")) {
                            ...
                            if (canvas) return el.appendChild(canvas),
                            canvas = null,
                            img.destroy(),
                            void self.trigger("embedded")
                        }
                        var dataUrl = img.getAsDataURL(type, quality);
                        if (!dataUrl) throw new x.ImageError(x.ImageError.WRONG_FORMAT);
                        if (Env.can("use_data_uri_of", dataUrl.length)) el.innerHTML = '<img src="' + dataUrl + '" width="' + img.width + '" height="' + img.height + '" />',
                        img.destroy(),
                        self.trigger("embedded");
                        else {
                            var tr = new Transporter;
                            tr.bind("TransportingComplete",
                            function() {
                    ...
                    try {
                        ...
                        this.meta.thumb && this.meta.thumb.width >= opts.width && this.meta.thumb.height >= opts.height ? imgCopy.loadTC(this.meta.thumb.data) : imgCopy.clone(this, !1),
                        imgCopy
                    } catch(ex) {
                        this.trigger("error", ex.code)
                    }
            ...
            this.handleEventProps(dispatches),
            this.bind("loadTC Resize",
            function() {
                _updateInfo.call(this)
            },
            999)
        }
        var dispatches = ["progressTC", "loadTC", "error", "resize", "embedded"];
        return Image.MAX_RESIZE_WIDTH = 8192,
        Image.MAX_RESIZE_HEIGHT = 8192,
        Image.prototype = EventTarget.instance,
        Image
    }),
    define("moxie/runtime/html5/Runtime", ["moxie/core/utils/Basic", "moxie/core/Exceptions", "moxie/runtime/Runtime", "moxie/core/utils/Env"],
    function(Basic, x, Runtime, Env) {
        ...
        var type = "html5",
        extensions = {};
        return Runtime.addConstructor(type, Html5Runtime),
        extensions
    }),
    define("moxie/core/utils/Events", ["moxie/core/utils/Basic"],
    function(Basic) {
        ...
        addEvent = function(obj, name, callback, key) {
            ...
            obj.addEventListener ? (func = callback, obj.addEventListener(name, func, !1)) : obj.attachEvent && (func = function() {
            ...
            obj[uid] || (obj[uid] = Basic.guid()),
            eventhash.hasOwnProperty(obj[uid]) || (eventhash[obj[uid]] = {}),
            events = eventhash[obj[uid]],
            events.hasOwnProperty(name) || (events[name] = []),
            events[name].push({
                func: func,
                orig: callback,
                key: key
        ...
        removeAllEvents = function(obj, key) {
            obj && obj[uid] && Basic.each(eventhash[obj[uid]],
    ...
    define("moxie/runtime/html5/file/FileInput", ["moxie/runtime/html5/Runtime", "moxie/file/File", "moxie/core/utils/Basic", "moxie/core/utils/Dom", "moxie/core/utils/Events", "moxie/core/utils/Mime", "moxie/core/utils/Env"],
        ...
        function FileInput() {
            ...
            Basic.extend(this, {
                init: function(options) {
                    ...
                    I.can("summon_file_dialog") && ("static" === Dom.getStyle(browseButton, "position") && (browseButton.style.position = "relative"), zIndex = parseInt(Dom.getStyle(browseButton, "z-index"), 10) || 1, browseButton.style.zIndex = zIndex, shimContainer.style.zIndex = zIndex - 1, Events.addEvent(browseButton, "click",
                    function(e) {
                        var input = Dom.get(I.uid);
                        input && !input.disabled && input.click(),
                        e.preventDefault()
                    },
                    comp.uid)),
                    top = I.can("summon_file_dialog") ? browseButton: shimContainer,
                    Events.addEvent(top, "mouseover",
                    function() {
                        comp.trigger("mouseenter")
                    },
                    comp.uid),
                    Events.addEvent(top, "mouseout",
                    function() {
                        comp.trigger("mouseleave")
                    },
                    comp.uid),
                    Events.addEvent(top, "mousedown",
                    function() {
                        comp.trigger("mousedown")
                    },
                    comp.uid),
                    Events.addEvent(Dom.get(_options.container), "mouseup",
                    function() {
                        comp.trigger("mouseup")
                    },
                    comp.uid),
                    input.onchange = function onChange(e) {
                        if (comp.files = [], Basic.each(this.files,
                        function(file) {
                            var relativePath = "";
                ...
                disable: function(state) {
                    var input, I = this.getRuntime(); (input = Dom.get(I.uid)) && (input.disabled = !!state)
                },
                destroy: function() {
                    ...
                    shimContainer && (shimContainer.innerHTML = ""),
    ...
    define("moxie/runtime/html5/file/Blob", ["moxie/runtime/html5/Runtime", "moxie/file/Blob"],
    ...
    define("moxie/runtime/html5/file/FileDrop", ["moxie/runtime/html5/Runtime", "moxie/file/File", "moxie/core/utils/Basic", "moxie/core/utils/Dom", "moxie/core/utils/Events", "moxie/core/utils/Mime"],
        ...
        function FileDrop() {
            ...
            Basic.extend(this, {
                init: function(options) {
                    ...
                    Events.addEvent(dropZone, "dragover",
                    function(e) {
                        _hasFiles(e) && (e.preventDefault(), e.dataTransfer.dropEffect = "copy")
                    },
                    comp.uid),
                    Events.addEvent(dropZone, "drop",
                    function(e) {
                        _hasFiles(e) && (e.preventDefault(), _files = [], e.dataTransfer.items && e.dataTransfer.items[0].webkitGetAsEntry ? _readItems(e.dataTransfer.items,
                        function() {
                            comp.files = _files,
                            comp.trigger("drop")
                        }) : (Basic.each(e.dataTransfer.files,
                        function(file) {
                            _addFile(file)
                        }), comp.files = _files, comp.trigger("drop")))
                    },
                    comp.uid),
                    Events.addEvent(dropZone, "dragenter",
                    function(e) {
                        comp.trigger("dragenter")
                    },
                    comp.uid),
                    Events.addEvent(dropZone, "dragleave",
                    function(e) {
    ...
    define("moxie/runtime/html5/file/FileReader", ["moxie/runtime/html5/Runtime", "moxie/core/utils/Encode", "moxie/core/utils/Basic"],
        ...
        function FileReader() {
            ...
            Basic.extend(this, {
                read: function(op, blob) {
                    ...
                    _fr.addEventListener("progressTC",
                    function(e) {
                        comp.trigger(e)
                    }),
                    _fr.addEventListener("loadTC",
                    function(e) {
                    ...
                    _fr.addEventListener("loadendTC",
                    function(e) {
                        _fr = null,
                        comp.trigger(e)
                    }),
    ...
    define("moxie/runtime/html5/xhr/XMLHttpRequest", ["moxie/runtime/html5/Runtime", "moxie/core/utils/Basic", "moxie/core/utils/Mime", "moxie/core/utils/Url", "moxie/file/File", "moxie/file/Blob", "moxie/xhr/FormData", "moxie/core/Exceptions", "moxie/core/utils/Env"],
    function(extensions, Basic, Mime, Url, File, Blob, FormData, x, Env) {
        function XMLHttpRequest() {
            ...
            function _getDocument(xhr) {
                ...
                return "IE" === Env.browser && rText && rXML && !rXML.documentElement && /[^\/]+\/[^\+]+\+xml/.test(xhr.getResponseHeader("Content-Type")) && (rXML = new window.ActiveXObject("Microsoft.XMLDOM"), rXML.async = !1, rXML.validateOnParse = !1, rXML.loadXML(rText)),
                rXML && ("IE" === Env.browser && 0 !== rXML.parseError || !rXML.documentElement || "parsererror" === rXML.documentElement.tagName) ? null: rXML
            }
            function _prepareMultipart(fd) {
                ...
                return _xhr.setRequestHeader("Content-Type", "multipart/form-data; boundary=" + boundary),
            ...
            Basic.extend(this, {
                send: function(meta, data) {
                    ...
                    mustSendAsBinary = !1;
                    if (_filename = meta.url.replace(/^.+?\/([\w\-\.]+)$/, "$1").toLowerCase(), _xhr = _getNativeXHR(), _xhr.open(meta.method, meta.url, meta.async, meta.user, meta.password), data instanceof Blob) data.isDetached() && (mustSendAsBinary = !0),
                    data = data.getSource();
                    else if (data instanceof FormData) {
                        if (data.hasBlob()) if (data.getBlob().isDetached()) data = _prepareMultipart.call(target, data),
                        mustSendAsBinary = !0;
                    ...
                    _xhr.upload ? (meta.withCredentials && (_xhr.withCredentials = !0), _xhr.addEventListener("loadTC",
                    function(e) {
                        target.trigger(e)
                    }), _xhr.addEventListener("error",
                    function(e) {
                        target.trigger(e)
                    }), _xhr.addEventListener("progressTC",
                    function(e) {
                        target.trigger(e)
                    }), _xhr.upload.addEventListener("progressTC",
                    function(e) {
                        target.trigger({
                            type: "UploadProgress",
                            loaded: e.loaded,
                        ...
                        case 3:
                            var total, loaded;
                            try {
                                Url.hasSameOrigin(meta.url) && (total = _xhr.getResponseHeader("Content-Length") || 0),
                                _xhr.responseText && (loaded = _xhr.responseText.length)
                            } catch(ex) {
                                total = loaded = 0
                            }
                            target.trigger({
                        ...
                        case 4:
                            _xhr.onreadystatechange = function() {},
                            0 === _xhr.status ? target.trigger("error") : target.trigger("loadTC")
                        }
                    },
                    Basic.isEmptyObj(meta.headers) || Basic.each(meta.headers,
                    function(value, header) {
                        _xhr.setRequestHeader(header, value)
                    }),
                    "" !== meta.responseType && "responseType" in _xhr && ("json" !== meta.responseType || Env.can("return_response_type", "json") ? _xhr.responseType = meta.responseType: _xhr.responseType = "text"),
                    mustSendAsBinary ? _xhr.sendAsBinary ? _xhr.sendAsBinary(data) : !
                    function() {
                ...
                getResponse: function(responseType) {
                    ...
                    try {
                        ...
                        case "blob":
                            ...
                            file.type || (file.type = Mime.getFileMime(_filename)),
                            file;
                        case "json":
    ...
    define("moxie/runtime/html5/utils/BinaryReader", ["moxie/core/utils/Basic"],
    function(Basic) {
                ...
                SEGMENT: function(idx, size, value) {
                    ...
                    case 3:
                        if (null === value && (value = new ArrayBuffer), value instanceof ArrayBuffer) {
                            ...
                            idx > 0 && arr.set(new Uint8Array(data.slice(0, idx)), 0),
        ...
        function UTF16StringReader(data) {
            function putstr(segment, idx, length) {
                length = 3 === arguments.length ? length: data.length - idx - 1,
    ...
    define("moxie/runtime/html5/image/JPEGHeaders", ["moxie/runtime/html5/utils/BinaryReader", "moxie/core/Exceptions"],
    function(BinaryReader, x) {
        return function JPEGHeaders(data) {
            var _br, idx, marker, headers = [],
            length = 0;
            if (_br = new BinaryReader(data), 65496 !== _br.SHORT(0)) throw _br.clear(),
            new x.ImageError(x.ImageError.WRONG_FORMAT);
            for (idx = 2; idx <= _br.length();) if (marker = _br.SHORT(idx), marker >= 65488 && 65495 >= marker) idx += 2;
            else {
                if (65498 === marker || 65497 === marker) break;
                length = _br.SHORT(idx + 2) + 2,
                marker >= 65505 && 65519 >= marker && headers.push({
                    hex: marker,
                    name: "APP" + (15 & marker),
                    start: idx,
                    length: length,
            ...
            return _br.clear(),
            {
                ...
                restore: function(data) {
                    var max, i, br;
                    for (br = new BinaryReader(data), idx = 65504 == br.SHORT(2) ? 4 + br.SHORT(4) : 2, i = 0, max = headers.length; max > i; i++) br.SEGMENT(idx, 0, headers[i].segment),
                    idx += headers[i].length;
                    return data = br.SEGMENT(),
                    br.clear(),
                    data
                },
                strip: function(data) {
    ...
    define("moxie/runtime/html5/image/ExifParser", ["moxie/core/utils/Basic", "moxie/runtime/html5/utils/BinaryReader", "moxie/core/Exceptions"],
    function(Basic, BinaryReader, x) {
        function ExifParser(data) {
                ...
                EXIF: function() {
                    ...
                    if (offsets.exifIFD) {
                        ...
                        if (Exif.ExifVersion && "array" === Basic.typeOf(Exif.ExifVersion)) {
                            for (var i = 0,
                            exifVersion = ""; i < Exif.ExifVersion.length; i++) exifVersion += String.fromCharCode(Exif.ExifVersion[i]);
            ...
            "ExifIFDPointer" in Tiff && (offsets.exifIFD = offsets.tiffHeader + Tiff.ExifIFDPointer, delete Tiff.ExifIFDPointer),
            "GPSInfoIFDPointer" in Tiff && (offsets.gpsIFD = offsets.tiffHeader + Tiff.GPSInfoIFDPointer, delete Tiff.GPSInfoIFDPointer),
            Basic.isEmptyObj(Tiff) && (Tiff = null);
            var IFD1Offset = this.LONG(offsets.IFD0 + 12 * this.SHORT(offsets.IFD0) + 2);
            IFD1Offset && (offsets.IFD1 = offsets.tiffHeader + IFD1Offset)
        }
        return ExifParser.prototype = BinaryReader.prototype,
        ExifParser
    }),
    define("moxie/runtime/html5/image/JPEG", ["moxie/core/utils/Basic", "moxie/core/Exceptions", "moxie/runtime/html5/image/JPEGHeaders", "moxie/runtime/html5/utils/BinaryReader", "moxie/runtime/html5/image/ExifParser"],
    function(Basic, x, JPEGHeaders, BinaryReader, ExifParser) {
        function JPEG(data) {
            ...
            Basic.extend(this, {
                ...
                setExif: function(tag, value) {
                    return _ep ? ("object" === Basic.typeOf(tag) ? Basic.each(tag,
                    function(value, tag) {
    ...
    define("moxie/runtime/html5/image/PNG", ["moxie/core/Exceptions", "moxie/core/utils/Basic", "moxie/runtime/html5/utils/BinaryReader"],
    function(x, Basic, BinaryReader) {
        function PNG(data) {
            function _getDimensions() {
                var chunk, idx;
                return chunk = _getChunkAt.call(this, 8),
                "IHDR" == chunk.type ? (idx = chunk.start, {
                    width: _br.LONG(idx),
    ...
    define("moxie/runtime/html5/image/ImageInfo", ["moxie/core/utils/Basic", "moxie/core/Exceptions", "moxie/runtime/html5/image/JPEG", "moxie/runtime/html5/image/PNG"],
    ...
    define("moxie/runtime/html5/image/MegaPixel", [],
    function() {
        ...
        function detectSubsampling(img) {
            ...
            if (iw * ih > 1048576) {
                ...
                return ctx.drawImage(img, -iw + 1, 0),
    ...
    define("moxie/runtime/html5/image/Image", ["moxie/runtime/html5/Runtime", "moxie/core/utils/Basic", "moxie/core/Exceptions", "moxie/core/utils/Encode", "moxie/file/Blob", "moxie/file/File", "moxie/runtime/html5/image/ImageInfo", "moxie/runtime/html5/image/MegaPixel", "moxie/core/utils/Mime", "moxie/core/utils/Env"],
    function(extensions, Basic, x, Encode, Blob, File, ImageInfo, MegaPixel, Mime, Env) {
        function HTML5Image() {
            ...
            function _downsize(width, height, crop, preserveHeaders) {
                ...
                return img = _getImg(),
                crop ? (width = Math.min(width, img.width), height = Math.min(height, img.height), scale = Math.max(width / img.width, height / img.height)) : scale = Math.min(width / img.width, height / img.height),
                scale > 1 && !crop && preserveHeaders ? void this.trigger("Resize") : (_canvas || (_canvas = document.createElement("canvas")), destWidth = Math.round(img.width * scale), destHeight = Math.round(img.height * scale), crop ? (_canvas.width = width, _canvas.height = height, destWidth > width && (x = Math.round((destWidth - width) / 2)), destHeight > height && (y = Math.round((destHeight - height) / 2))) : (_canvas.width = destWidth, _canvas.height = destHeight), _preserveHeaders || _rotateToOrientaion(_canvas.width, _canvas.height, orientation), _drawToCanvas.call(this, img, _canvas, -x, -y, destWidth, destHeight), this.width = _canvas.width, this.height = _canvas.height, _modified = !0, void self.trigger("Resize"))
            }
            ...
            function _purge() {
                _imgInfo && (_imgInfo.purge(), _imgInfo = null),
            ...
            Basic.extend(this, {
                loadFromBlob: function(blob) {
                    ...
                    return _blob = blob,
                    blob.isDetached() ? (_binStr = blob.getSource(), void _preload.call(this, _binStr)) : void _readAsDataUrl.call(this, blob.getSource(),
                    function(dataUrl) {
                        asBinary && (_binStr = _toBinary(dataUrl)),
                ...
                getInfo: function() {
                    ...
                    return ! _imgInfo && _binStr && I.can("access_image_binary") && (_imgInfo = new ImageInfo(_binStr)),
                    info = {
                ...
                getAsCanvas: function() {
                    return _canvas && (_canvas.id = this.uid + "_canvas"),
                    _canvas
                },
                getAsBlob: function(type, quality) {
                    return type !== this.type && _downsize.call(this, this.width, this.height, !1),
                    new File(null, {
                        name: _blob.name || "",
                        type: type,
                ...
                getAsBinaryString: function(type, quality) {
                    if (!_modified) return _binStr || (_binStr = _toBinary(me.getAsDataURL(type, quality))),
                    _binStr;
                    if ("image/jpeg" !== type) _binStr = _toBinary(me.getAsDataURL(type, quality));
                        ...
                        _imgInfo && (_binStr = _imgInfo.stripHeaders(_binStr), _preserveHeaders && (_imgInfo.meta && _imgInfo.meta.exif && _imgInfo.setExif({
    ...
    define("moxie/runtime/flash/Runtime", ["moxie/core/utils/Basic", "moxie/core/utils/Env", "moxie/core/utils/Dom", "moxie/core/Exceptions", "moxie/runtime/Runtime"],
    function(Basic, Env, Dom, x, Runtime) {
        function getShimVersion() {
            ...
            return version = version.match(/\d+/g),
            parseFloat(version[0] + "." + version[1])
        }
        function removeSWF(id) {
            var obj = Dom.get(id);
            obj && "OBJECT" == obj.nodeName && ("IE" === Env.browser ? (obj.style.display = "none",
            function onInit() {
        ...
        function FlashRuntime(options) {
            ...
            "client"),
            getShimVersion() < 10 && (MXI_DEBUG && Env.debug.runtime && Env.log("	Flash didn't meet minimal version requirement (10)."), this.mode = !1),
            Basic.extend(this, {
                getShim: function() {
                ...
                init: function() {
                    ...
                    "IE" === Env.browser && (html += 'classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" '),
                    ...
                    "IE" === Env.browser ? (el = document.createElement("div"), container.appendChild(el), el.outerHTML = html, el = container = null) : container.innerHTML = html,
        ...
        var type = "flash",
        extensions = {};
        return Runtime.addConstructor(type, FlashRuntime),
        extensions
    }),
    define("moxie/runtime/flash/file/FileInput", ["moxie/runtime/flash/Runtime", "moxie/file/File", "moxie/core/utils/Basic"],
    ...
    define("moxie/runtime/flash/file/Blob", ["moxie/runtime/flash/Runtime", "moxie/file/Blob"],
    function(extensions, Blob) {
        var FlashBlob = {
            slice: function(blob, start, end, type) {
                var self = this.getRuntime();
                return 0 > start ? start = Math.max(blob.size + start, 0) : start > 0 && (start = Math.min(start, blob.size)),
                0 > end ? end = Math.max(blob.size + end, 0) : end > 0 && (end = Math.min(end, blob.size)),
                blob = self.shimExec.call(this, "Blob", "slice", start, end, type || ""),
                blob && (blob = new Blob(self.uid, blob)),
                blob
            }
        };
        return extensions.Blob = FlashBlob
    }),
    define("moxie/runtime/flash/file/FileReader", ["moxie/runtime/flash/Runtime", "moxie/core/utils/Encode"],
    function(extensions, Encode) {
        ...
        var FileReader = {
            read: function(op, blob) {
                var comp = this;
                return comp.result = "",
                "readAsDataURL" === op && (comp.result = "data:" + (blob.type || "") + ";base64,"),
                comp.bind("progressTC",
                function(e, data) {
    ...
    define("moxie/runtime/flash/file/FileReaderSync", ["moxie/runtime/flash/Runtime", "moxie/core/utils/Encode"],
    function(extensions, Encode) {
        function _formatData(data, op) {
            switch (op) {
            case "readAsText":
    ...
    function(extensions, Basic, Blob, File, FileReaderSync, FormData, Transporter) {
        var XMLHttpRequest = {
            send: function(meta, data) {
                ...
                if (Basic.isEmptyObj(meta.headers) || Basic.each(meta.headers,
                function(value, header) {
                    ...
                    if (data.each(function(value, name) {
                        ...
                        var blob = data.getBlob();
                        blob.isDetached() ? attachBlob(blob,
                        function(attachedBlob) {
                            blob.destroy(),
                            appendBlob(blobField, attachedBlob)
                        }) : appendBlob(blobField, blob)
                    } else data = null,
                    send()
                } else data instanceof Blob ? data.isDetached() ? attachBlob(data,
                function(attachedBlob) {
                    data.destroy(),
    ...
    define("moxie/runtime/flash/runtime/Transporter", ["moxie/runtime/flash/Runtime", "moxie/file/Blob"],
    function(extensions, Blob) {
        var Transporter = {
    ...
    define("moxie/runtime/flash/image/Image", ["moxie/runtime/flash/Runtime", "moxie/core/utils/Basic", "moxie/runtime/Transporter", "moxie/file/Blob", "moxie/file/FileReaderSync"],
    function(extensions, Basic, Transporter, Blob, FileReaderSync) {
        var Image = {
            ...
            getInfo: function() {
                ...
                return ! info.meta || !info.meta.thumb || info.meta.thumb.data instanceof Blob || (info.meta.thumb.data = new Blob(self.uid, info.meta.thumb.data)),
                info
            },
            getAsBlob: function(type, quality) {
                var self = this.getRuntime(),
    ...
    define("moxie/runtime/silverlight/Runtime", ["moxie/core/utils/Basic", "moxie/core/utils/Env", "moxie/core/utils/Dom", "moxie/core/Exceptions", "moxie/runtime/Runtime"],
    function(Basic, Env, Dom, x, Runtime) {
            ...
            try {
                try {
                    ...
                    if (plugin) {
                        ...
                        do requiredVersionPart = parseInt(reqVerArray[index], 10),
        ...
        function SilverlightRuntime(options) {
            ...
            isInstalled("2.0.31005.0") && "Opera" !== Env.browser || (MXI_DEBUG && Env.debug.runtime && Env.log("	Silverlight is not installed or minimal version (2.0.31005.0) requirement not met (not likely)."), this.mode = !1),
            Basic.extend(this, {
                getShim: function() {
        ...
        extensions = {};
        return Runtime.addConstructor(type, SilverlightRuntime),
        extensions
    }),
    define("moxie/runtime/silverlight/file/FileInput", ["moxie/runtime/silverlight/Runtime", "moxie/file/File", "moxie/core/utils/Basic"],
    function(extensions, File, Basic) {
        var FileInput = {
            init: function(options) {
                function toFilters(accept) {
                    for (var filter = "",
                    i = 0; i < accept.length; i++) filter += ("" !== filter ? "|": "") + accept[i].title + " | *." + accept[i].extensions.replace(/,/g, ";*.");
                    return filter
                }
                var comp = this,
                I = this.getRuntime();
    ...
    define("moxie/runtime/silverlight/file/Blob", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Basic", "moxie/runtime/flash/file/Blob"],
    function(extensions, Basic, Blob) {
        return extensions.Blob = Basic.extend({},
        Blob)
    }),
    define("moxie/runtime/silverlight/file/FileDrop", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Dom", "moxie/core/utils/Events"],
    ...
    define("moxie/runtime/silverlight/file/FileReader", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Basic", "moxie/runtime/flash/file/FileReader"],
    function(extensions, Basic, FileReader) {
        return extensions.FileReader = Basic.extend({},
        FileReader)
    }),
    define("moxie/runtime/silverlight/file/FileReaderSync", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Basic", "moxie/runtime/flash/file/FileReaderSync"],
    function(extensions, Basic, FileReaderSync) {
        return extensions.FileReaderSync = Basic.extend({},
        FileReaderSync)
    }),
    define("moxie/runtime/silverlight/xhr/XMLHttpRequest", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Basic", "moxie/runtime/flash/xhr/XMLHttpRequest"],
    function(extensions, Basic, XMLHttpRequest) {
        return extensions.XMLHttpRequest = Basic.extend({},
        XMLHttpRequest)
    }),
    define("moxie/runtime/silverlight/runtime/Transporter", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Basic", "moxie/runtime/flash/runtime/Transporter"],
    function(extensions, Basic, Transporter) {
        return extensions.Transporter = Basic.extend({},
        Transporter)
    }),
    define("moxie/runtime/silverlight/image/Image", ["moxie/runtime/silverlight/Runtime", "moxie/core/utils/Basic", "moxie/file/Blob", "moxie/runtime/flash/image/Image"],
    function(extensions, Basic, Blob, Image) {
        ...
        Image, {
            getInfo: function() {
                ...
                return rawInfo.meta && (Basic.each(grps,
                function(grp) {
                    var tag, i, length, value, meta = rawInfo.meta[grp];
                    if (meta && meta.keys) for (info.meta[grp] = {},
                    i = 0, length = meta.keys.length; length > i; i++) tag = meta.keys[i],
                    value = meta[tag],
                    value && (/^(\d|[1-9]\d+)$/.test(value) ? value = parseInt(value, 10) : /^\d*\.\d+$/.test(value) && (value = parseFloat(value)), info.meta[grp][tag] = value)
                }), !info.meta || !info.meta.thumb || info.meta.thumb.data instanceof Blob || (info.meta.thumb.data = new Blob(self.uid, info.meta.thumb.data))),
    ...
    define("moxie/runtime/html4/Runtime", ["moxie/core/utils/Basic", "moxie/core/Exceptions", "moxie/runtime/Runtime", "moxie/core/utils/Env"],
    function(Basic, x, Runtime, Env) {
        ...
        var type = "html4",
        extensions = {};
        return Runtime.addConstructor(type, Html4Runtime),
        extensions
    }),
    define("moxie/runtime/html4/file/FileInput", ["moxie/runtime/html4/Runtime", "moxie/file/File", "moxie/core/utils/Basic", "moxie/core/utils/Dom", "moxie/core/utils/Events", "moxie/core/utils/Mime", "moxie/core/utils/Env"],
    function(extensions, File, Basic, Dom, Events, Mime, Env) {
        function FileInput() {
            function addInput() {
                ...
                uid = Basic.guid("uid_"),
                shimContainer = I.getShimContainer(),
                _uid && (currForm = Dom.get(_uid + "_form"), currForm && Basic.extend(currForm.style, {
                    top: "100%"
                })),
                ...
                I.can("summon_file_dialog") && (browseButton = Dom.get(_options.browse_button), Events.removeEvent(browseButton, "click", comp.uid), Events.addEvent(browseButton, "click",
                function(e) {
                    input && !input.disabled && input.click(),
                    e.preventDefault()
                },
            ...
            Basic.extend(this, {
                init: function(options) {
                    ...
                    function() {
                        ...
                        browseButton = Dom.get(options.browse_button),
                        I.can("summon_file_dialog") && ("static" === Dom.getStyle(browseButton, "position") && (browseButton.style.position = "relative"), zIndex = parseInt(Dom.getStyle(browseButton, "z-index"), 10) || 1, browseButton.style.zIndex = zIndex, shimContainer.style.zIndex = zIndex - 1),
                        top = I.can("summon_file_dialog") ? browseButton: shimContainer,
                        Events.addEvent(top, "mouseover",
                        function() {
                            comp.trigger("mouseenter")
                        ...
                        comp.uid),
                        Events.addEvent(Dom.get(options.container), "mouseup",
                        function() {
                            comp.trigger("mouseup")
                        },
                        comp.uid),
                ...
                disable: function(state) {
                    var input; (input = Dom.get(_uid)) && (input.disabled = !!state)
                },
                destroy: function() {
                    var I = this.getRuntime(),
                    shim = I.getShim(),
    ...
    define("moxie/runtime/html4/file/FileReader", ["moxie/runtime/html4/Runtime", "moxie/runtime/html5/file/FileReader"],
    function(extensions, FileReader) {
        return extensions.FileReader = FileReader
    }),
    define("moxie/runtime/html4/xhr/XMLHttpRequest", ["moxie/runtime/html4/Runtime", "moxie/core/utils/Basic", "moxie/core/utils/Dom", "moxie/core/utils/Url", "moxie/core/Exceptions", "moxie/core/utils/Events", "moxie/file/Blob", "moxie/xhr/FormData"],
            ...
            function cleanup(cb) {
                ...
                if (_iframe) {
                    if (uid = _iframe.id.replace(/_iframe$/, ""), form = Dom.get(uid + "_form")) {
                        ...
                        inputs = [],
                        hasFile || form.parentNode.removeChild(form),
                        form = null
                    }
                    setTimeout(function() {
                        Events.removeEvent(_iframe, "loadTC", target.uid),
                        _iframe.parentNode && _iframe.parentNode.removeChild(_iframe);
                        var shimContainer = target.getRuntime().getShimContainer();
                        shimContainer.children.length || shimContainer.parentNode.removeChild(shimContainer),
            ...
            Basic.extend(this, {
                send: function(meta, data) {
                    function createIframe() {
                        ...
                        Events.addEvent(_iframe, "loadTC",
                        function() {
                            var el;
                            try {
                                el = _iframe.contentWindow.document || _iframe.contentDocument || window.frames[_iframe.id].document,
                                /^4(0[0-9]|1[0-7]|2[2346])\s/.test(el.title) ? _status = el.title.replace(/^(\d+).*$/, "$1") : (_status = 200, _response = Basic.trim(el.body.innerHTML), target.trigger({
                                    type: "progressTC",
                                ...
                                if (!Url.hasSameOrigin(meta.url)) return void cleanup.call(target,
                                function() {
                    ...
                    data instanceof FormData && data.each(function(value, name) {
                ...
                abort: function() {
                    var target = this;
                    _iframe && _iframe.contentWindow && (_iframe.contentWindow.stop ? _iframe.contentWindow.stop() : _iframe.contentWindow.document.execCommand ? _iframe.contentWindow.document.execCommand("Stop") : _iframe.src = "about:blank"),
                    cleanup.call(this,
                    function() {
    ...
    define("moxie/runtime/html4/image/Image", ["moxie/runtime/html4/Runtime", "moxie/runtime/html5/image/Image"],
...
function(exports) {
    ...
    return function addAlias(ns) {
        var name, itemType;
        for (name in ns) itemType = typeof ns[name],
        "object" !== itemType || ~inArray(name, ["Exceptions", "Env", "Mime"]) ? "function" === itemType && (o[name] = ns[name]) : addAlias(ns[name])
    } (exports.moxie),
    o.Env = exports.moxie.core.utils.Env,
    o.Mime = exports.moxie.core.utils.Mime,
    o.Exceptions = exports.moxie.core.Exceptions,
    exports.mOxie = o,
    exports.o || (exports.o = o),
    o
} (this),
function(window, o, undef) {
    function normalizeCaps(settings) {
        function resolve(feature, value, strict) {
            var map = {
                ...
                progressTC: "report_upload_progress",
        ...
        caps = {};
        return "string" == typeof features ? plupload.each(features.split(/\s*,\s*/),
        function(feature) {
            resolve(feature, !0)
        }) : "object" == typeof features ? plupload.each(features,
        function(value, feature) {
            resolve(feature, value)
        }) : features === !0 && (settings.chunk_size > 0 && (caps.slice_blob = !0), (settings.resize.enabled || !settings.multipart) && (caps.send_binary_string = !0), plupload.each(settings,
        function(value, feature) {
    ...
    plupload = {
        ...
        getAll: function(ids) {
            ...
            for (var i = ids.length; i--;) el = plupload.get(ids[i]),
            el && els.push(el);
        ...
        xmlEncode: function(str) {
            ...
            xmlEncodeRegExp = /[<>&\"\']/g;
            return str ? ("" + str).replace(xmlEncodeRegExp,
            function(chr) {
        ...
        buildUrl: function(url, items) {
            var query = "";
            return plupload.each(items,
            function(value, name) {
                query += (query ? "&": "") + encodeURIComponent(name) + "=" + encodeURIComponent(value)
            }),
            query && (url += (url.indexOf("?") > 0 ? "&": "?") + query),
            url
        },
        ...
        predictRuntime: function(config, runtimes) {
            var up, runtime;
            return up = new plupload.Uploader(config),
    ...
    plupload.addFileFilter("mime_types",
    function(filters, file, cb) {
        filters.length && !filters.regexp.test(file.name) ? (this.trigger("error", {
            code: plupload.FILE_EXTENSION_ERROR,
            messageTC: plupload.translate("File extension error."),
            file: file
        }), cb(!1)) : cb(!0)
    }),
    plupload.addFileFilter("max_file_size",
    function(maxSize, file, cb) {
        var undef;
        maxSize = plupload.parseSize(maxSize),
        file.size !== undef && maxSize && file.size > maxSize ? (this.trigger("error", {
            code: plupload.FILE_SIZE_ERROR,
            messageTC: plupload.translate("File size error."),
            file: file
        }), cb(!1)) : cb(!0)
    }),
    plupload.addFileFilter("prevent_duplicates",
    function(value, file, cb) {
        if (value) for (var ii = this.files.length; ii--;) if (file.name === this.files[ii].name && file.size === this.files[ii].size) return this.trigger("error", {
            ...
            messageTC: plupload.translate("Duplicate file error."),
            file: file
        }),
        void cb(!1);
        cb(!0)
    ...
    plupload.Uploader = function(options) {
        ...
        function calc() {
            var i, file;
            for (total.reset(), i = 0; i < files.length; i++) file = files[i],
            file.size !== undef ? (total.size += file.origSize, total.loaded += file.loaded * file.origSize / file.size) : total.size = undef,
            file.status == plupload.DONE ? total.uploaded++:file.status == plupload.FAILED ? total.failed++:total.queued++;
        ...
        function initControls(settings, cb) {
            ...
            plupload.each(settings.runtimes.split(/\s*,\s*/),
            function(runtime) {
                settings[runtime] && (options[runtime] = settings[runtime])
            }),
            settings.browse_button && plupload.each(settings.browse_button,
            function(el) {
                queue.push(function(cb) {
                    var fileInput = new o.FileInput(plupload.extend({},
                    options, {
                    ...
                    fileInput.bind("mouseenter mouseleave mousedown mouseup",
                    function(e) {
                    ...
                    fileInput.bind("mousedown",
                    function() {
                        self.trigger("Browse")
                    }),
                    fileInput.bind("error runtimeerror",
                    function() {
                        fileInput = null,
                        cb()
                    }),
            ...
            settings.drop_element && plupload.each(settings.drop_element,
            function(el) {
                queue.push(function(cb) {
                    var fileDrop = new o.FileDrop(plupload.extend({},
                    options, {
                        drop_zone: el
                    }));
                    ...
                    fileDrop.bind("error runtimeerror",
                    function() {
                        fileDrop = null,
                        cb()
                    }),
        ...
        function setOption(option, value, init) {
            function _setOption(option, value, init) {
                ...
                case "multipart":
                    ...
                    value || (settings.send_file_name = !0);
                    break;
                case "unique_names":
                    settings[option] = value,
                    value && (settings.send_file_name = !0);
                ...
                case "filters":
                    ...
                    init ? plupload.extend(settings.filters, value) : settings.filters = value,
                    value.mime_types && (settings.filters.mime_types.regexp = function(filters) {
                        var extensionsRegExp = [];
                        return plupload.each(filters,
                        function(filter) {
                            plupload.each(filter.extensions.split(/,/),
                            function(ext) { / ^\s * \ * \s * $ / .test(ext) ? extensionsRegExp.push("\\.*") : extensionsRegExp.push("\\." + ext.replace(new RegExp("[" + "/^$.*+?|()[]{}\\".replace(/./g, "\\$&") + "]", "g"), "\\$&"))
                            })
                        }),
            ...
            var self = this,
            reinitRequired = !1;
            "object" == typeof option ? plupload.each(option,
            function(value, option) {
                _setOption(option, value, init)
            }) : _setOption(option, value, init),
            init ? (settings.required_features = normalizeCaps(plupload.extend({},
            settings)), preferred_caps = normalizeCaps(plupload.extend({},
            settings, {
                required_features: !0
            }))) : reinitRequired && (self.trigger("Destroy"), initControls.call(self, settings,
            function(inited) {
                inited ? (self.runtime = o.Runtime.getInfo(getRUID()).type, self.trigger("Init", {
                    runtime: self.runtime
                }), self.trigger("PostInit")) : self.trigger("error", {
                    code: plupload.INIT_ERROR,
                    messageTC: plupload.translate("Init error.")
                })
            }))
        }
        ...
        function onUploadFile(up, file) {
            function handleError(errorCode) {
                retries-->0 ? delay(uploadNextChunk, 1e3) : (file.loaded = offset, up.trigger("error", {
                    ...
                    messageTC: plupload.translate("HTTP error."),
                    file: file,
            ...
            function uploadNextChunk() {
                function startXhr() {
                    ...
                    var tid = setTimeout(function() {
                        xhr && 4 == xhr.readyState || (handleError(plupload.NETWORK_TIMEOUT), __monitor(2605262))
                    },
                    12e4);
                    ...
                    xhr.onload = function() {
                        ...
                        if (xhr.responseText) try {
                            ...
                            if (0 != code) return __monitor(2605261),
                            __offset !== !1 && (offset = __offset),
                            handleError(retJson.code);
                            if (__offset !== !1 && 0 === __offset) return handleError( - 5e4)
                        } catch(xe) {}
                        retries = up.settings.max_retries,
                        currChunkSize < blob.size ? (chunkBlob.destroy(), offset += currChunkSize, file.loaded = Math.min(offset, blob.size), up.trigger("ChunkUploaded", file, {
                            offset: file.loaded,
                            total: blob.size,
                            response: xhr.responseText,
                            status: xhr.status,
                            responseHeaders: xhr.getAllResponseHeaders()
                        }), "Android Browser" === o.Env.browser && up.trigger("UploadProgress", file)) : file.loaded = file.size,
                        chunkBlob = formData = null,
                        !offset || offset >= blob.size ? (file.size != file.origSize && (blob.destroy(), blob = null), up.trigger("UploadProgress", file), file.status = plupload.DONE, up.trigger("FileUploaded", file, {
                            response: xhr.responseText,
                            status: xhr.status,
                            responseHeaders: xhr.getAllResponseHeaders()
                    ...
                    up.settings.multipart && features.multipart ? (xhr.open("post", url, !0), plupload.each(up.settings.headers,
                    function(value, name) {
                        xhr.setRequestHeader(name, value)
                    }), formData = new o.FormData, plupload.each(args,
                    function(value, name) {
                    ...
                    })) : (url = plupload.buildUrl(up.settings.url, args), xhr.open("post", url, !0), xhr.setRequestHeader("Content-Type", "application/octet-stream"), plupload.each(up.settings.headers,
                    function(value, name) {
                        xhr.setRequestHeader(name, value)
                    }), xhr.send(chunkBlob, {
                        runtime_order: up.settings.runtimes,
                ...
                var chunkBlob, formData, currChunkSize, args = plupload.extend({},
                up.settings.multipart_params);
                if (file.status === plupload.UPLOADING && up.state !== plupload.STOPPED) {
                    up.settings.send_file_name && (args.name = file.target_name || file.name),
                    file.speedInKB && (chunkSizeMax = file.speedInKB < 50 ? 524288 : file.speedInKB < 100 ? 1048576 : file.speedInKB < 250 ? 2097152 : file.speedInKB < 500 ? 4194304 : 5242880),
                    currChunkSize = 0 == file.loaded || file.is_from_resume ? chunkSizePart1: Math.min(file.currChunkSize + chunkSizeStep, chunkSizeMax),
                    delete file.is_from_resume,
                    currChunkSize = Math.min(currChunkSize, blob.size - offset),
                    chunkBlob = blob.slice(offset, offset + currChunkSize),
                    file.currChunkSize = currChunkSize;
                    var waitSignature = !1;
                    if (currChunkSize && features.chunks) {
                        up.settings.send_chunk_number ? args.offset = offset: (args.offset = offset, args.total = blob.size),
                        args.dataSize = currChunkSize ? currChunkSize: 0,
                        args.Action = "MultipartUploadVodFile",
                        args.Region = "gz",
                        args.Timestamp = String(parseInt( + new Date / 1e3)),
                        args.Nonce = Math.ceil(1e4 * Math.random()),
                        args.utype = 1,
                        extBusiness.transcodeNotifyUrl && (args.notifyUrl = extBusiness.transcodeNotifyUrl),
                        file.hasOwnProperty("isTranscode") && (args.isTranscode = file.isTranscode),
                        file.hasOwnProperty("isWatermark") && (args.isWatermark = file.isWatermark),
                        file.hasOwnProperty("classId") && (args.classId = file.classId),
                        args = ksort(args);
                    ...
                    waitSignature ? up.settings.signature(sigString,
                    function(Signature) {
            ...
            offset = 0;
            file.currChunkStartUpTime = +new Date,
            file.loaded && (offset = file.loaded = chunkSizePart1 ? chunkSizePart1 * Math.floor(file.loaded / chunkSizePart1) : 0),
            blob = file.getSource(),
            up.settings.resize.enabled && runtimeCan(blob, "send_binary_string") && ~o.inArray(blob.type, ["image/jpeg", "image/png"]) ? resizeImage.call(this, blob, up.settings.resize,
        ...
        function onDestroy(up) {
            ...
            files = [],
            fileInputs.length && (plupload.each(fileInputs,
            function(fileInput) {
                fileInput.destroy()
            }), fileInputs = []),
            fileDrops.length && (plupload.each(fileDrops,
            function(fileDrop) {
                fileDrop.destroy()
            }), fileDrops = []),
            preferred_caps = {},
        ...
        setOption.call(this, options, null, !0),
        ...
        plupload.extend(this, {
            ...
            init: function() {
                ...
                return preinitOpt = self.getOption("preinit"),
                "function" == typeof preinitOpt ? preinitOpt(self) : plupload.each(preinitOpt,
                function(func, name) {
                    self.bind(name, func)
                }),
                bindEventListeners.call(self),
                plupload.each(["container", "browse_button", "drop_element"],
                function(el) {
                    return null === self.getOption(el) ? (err = {
                        code: plupload.INIT_ERROR,
                        messageTC: plupload.translate("'%' specified, but cannot be found.")
                    },
                    !1) : void 0
                }),
                err ? self.trigger("error", err) : settings.browse_button || settings.drop_element ? void initControls.call(self, settings,
                function(inited) {
                    var initOpt = self.getOption("init");
                    "function" == typeof initOpt ? initOpt(self) : plupload.each(initOpt,
                    function(func, name) {
                        self.bind(name, func)
                    }),
                    inited ? (self.runtime = o.Runtime.getInfo(getRUID()).type, self.trigger("Init", {
                    ...
                    code: plupload.INIT_ERROR,
                    messageTC: plupload.translate("You must specify either 'browse_button' or 'drop_element'.")
                })
            },
            ...
            disableBrowse: function() {
                disabled = arguments[0] !== undef ? arguments[0] : !0,
                fileInputs.length && plupload.each(fileInputs,
                function(fileInput) {
            ...
            addFile: function(file, fileName) {
                function filterFile(file, cb) {
                    var queue = [];
                    o.each(self.settings.filters,
                    function(rule, name) {
                        fileFilters[name] && queue.push(function(cb) {
                            fileFilters[name].call(self, rule, file,
                            function(res) {
                                cb(!res)
                            })
                        })
                ...
                function resolveFile(file) {
                    ...
                    if (file instanceof o.File) {
                        ...
                        filterFile(file,
                        function(err) {
                            err || (files.push(file), filesAdded.push(file), self.trigger("FileFiltered", file)),
                            delay(cb, 1)
                        })
            ...
            splice: function(start, length) {
                ...
                restartRequired = !1;
                return this.state == plupload.STARTED && (plupload.each(removed,
                function(file) {
                    return file.status === plupload.UPLOADING ? (restartRequired = !0, !1) : void 0
                }), restartRequired && this.stop()),
                this.trigger("FilesRemoved", removed),
                plupload.each(removed,
                function(file) {
                    file.destroy()
                }),
                restartRequired && this.start(),
                removed
            },
            dispatchEvent: function(type) {
                var list, args;
...
function(Log, $) {
    ...
    makeUploaderSwf = function(opt) {
        ...
        _('<object data="' + address + '" id="' + eid + '_object" width="' + opt.width + 'px" height="' + opt.height + 'px"  style="background-color:#000000;" '),
        ...
        _('<param name="flashVars" value="' + (opt.flashvars || "") + '"  />'),
        _('<param name="src" value="' + address + '"  />'),
        _('<param name="wmode" value="window"/>'),
        ...
        _('<embed style="background-color:#000000;"  id="' + eid + '_embed" width="' + opt.width + 'px" height="' + opt.height + 'px" flashvars="' + (opt.flashvars || "") + '"'),
        ...
        _('src="' + address + '"'),
        _('wmode="window" allowfullscreen="true" invokeurls="false" allownetworking="all" allowscriptaccess="always">'),
        _("</object>");
    ...
    $.extend(Worker.prototype, {
        ...
        eachMessageHandler: function(fn) {
            for (var list = this._event && this._event.messageTC ? this._event.messageTC: [], i = 0, j = list.length; j > i; i++) fn(list[i])
        },
        ...
        postMessage: function(event) {
            ...
            if (getASObject(function(aso) {
                aso.updateSHA(id, messageTC.slice(messageTC.indexOf("base64,") + 7))
            }), isEnd) {
                var id = this.getId();
...
function(util) {
    function jshash(str) {
        for (var hash = 1315423911,
        i = 0,
        length = str.length; length > i; i++) {
            var tmp = hash << 5 >>> 0;
    ...
    reporter = function() {
        function reportRet(params) {
            ...
            0 == params.action && (urlParams.cgi = "sha_calc"),
            report("https://huatuocode.huatuo.qq.com/code.cgi", urlParams)
        }
        ...
        report = function(url, params) {
            try {
                ...
                _url = _url.replace(/\/$/, ""),
                params._ = +new Date,
                _url = _url + "?" + _objectToParams(params, !0);
        ...
        return function(params) {
            ...
            params.appid = tool.cookie.get("appid") || secrectId || "11111",
            reportRet(params),
            0 == params.error_code && 1 != params.upload_flag && params.chunk_flag > 5 && params.cost_time < 30 || report(reportInterfaceUrl, params)
        }
    } (),
    Item = function(obj) {
        var data = this.data = {};
        data.browser = (obj.sdkver || "") + "_" + util.browser(),
        data.file_sha = 0,
        data.file_size = obj.size,
        data.file_name = obj.name,
        obj.secrectId && (secrectId = jshash(obj.secrectId)),
        this._reset()
    };
...
qcVideo("SHA",
function(Log, $, Version, AS3_Worker) {
    function hash_file(wrapFile, worker, bs) {
        function handle_err(event) {
            dispatchSHA(wrapFile, void 0, event)
        }
        var buffer_size, block, reader, handle_hash_block, handle_load_block, file = wrapFile.getNative(),
        fileId = wrapFile.id,
        clean = function() {
            Log.debug("SHA ，clear"),
        ...
        handle_load_block = function(event) {
            cache.ingMap[fileId] ? worker.postMessage({
                messageTC: event.target.result,
                block: block
            }) : (Log.debug("SHA ，继续计算时，发现已经被删除", fileId), clean(), startFileSha())
        },
        handle_hash_block = function(event) {
            block.end !== file.size ? (block.start += buffer_size, block.end += buffer_size, block.end > file.size && (block.end = file.size), reader && (reader.onload = null), reader = new FileReader, reader.onload = handle_load_block, reader.onerror = handle_err, readFile(reader, file, block)) : clean()
        },
        buffer_size = bs,
        block = {
            file_size: file.size,
        ...
        worker.addEventListener("messageTC", handle_hash_block),
        reader = new FileReader,
        reader.onload = handle_load_block,
        reader.onerror = handle_err,
    ...
    function handle_worker_event(file) {
        return function(event) {
            if (file.status == plupload.HASHING && cache.ingMap[file.id]) if (event.data.result) {
                ...
                file._report_timer && (clearInterval(file._report_timer), delete file.report_timer),
                file._hashed_size = file.size,
                Log.log("cost second :", Math.ceil(cost / 1e3), ";文件大小:", (file.size / Math.pow(2, 20)).toFixed(2) + "M", ";sha:", event.data.result),
                getReport(file).bSet({
                ...
                cache.ingMap[file.id] && (delete cache.ingMap[file.id], cache.ingMap.__length -= 1),
                startFileSha()
            } else {
                var block = event.data.block;
                file._hashed_size = block.end,
                SpeedLimit.set(file, (100 * block.end / block.file_size).toFixed(2))
            }
        }
    }
    function ableStart(file) {
        ...
        return calSize += file ? file.size: 0,
        0 === cache.ingMap.__length || SETTING.MAX_DOING_SHA1 > cache.ingMap.__length && SETTING.MAX_SHA_SIZE > calSize
    }
    function startFileSha() {
        ...
        if (length > 0 && ableStart(cache.waitList[0])) {
            ...
            worker.addEventListener("messageTC", handle_worker_event(file)),
            hash_file(file, worker, getBuffer(file))
        }
    }
    var SETTING = {
    ...
    SpeedLimit = function() {
        ...
        return {
            set: function(file, percent) {
                ...
                now - cache[id]._time > 1e3 && (cache[id]._time = now, dispatchSHA(file, percent))
            },
            del: function(id) {
                delete cache[id]
            }
    ...
    var handler = $.noop,
    dispatchSHA = function(file, percent, result) {
        handler(file, percent ? percent: 0, result)
    };
    return {
        ...
        add: function(file) {
            dispatchSHA(file, 0),
            ableStart(file) ? (cache.waitList = [file].concat(cache.waitList), startFileSha()) : cache.waitList.push(file)
        },
        del: function(fid) {
...
function(FILE_STATUS) {
    return {
        browser: function() {
            ...
            ret = "";
            return ret = /msie/i.test(br) && !/opera/.test(br) ? "IE": /firefox/i.test(br) ? "Firefox": /chrome/i.test(br) && /webkit/i.test(br) && /mozilla/i.test(br) ? "Chrome": /opera/i.test(br) ? "Opera": "ipad",
            (ret + " " + ver).replace(/\\s|\\./g, "_")
        },
        ...
        monitor: function(mid, p) {
            if (! ("undefined" != typeof p && Math.random() > p)) {
                var img = new Image;
                img.onload = img.onerror = img.onabort = function() {
                    img = img.onload = img.onerror = img.onabort = null
                },
                img.src = "https://video.qcloud.com/mreport?t=1&id=" + mid
            }
        }
    }
}),
...
function() {
    ...
    return Version.IS_MAC = window.navigator && navigator.appVersion && navigator.appVersion.indexOf("Mac") > -1,
    Version.IS_MOBILE = Version.IOS || Version.ANDROID,
    Version.FLASH_VERSION = -1,
    Version.IS_IE = "ActiveXObject" in window,
    Version.ABLE_FLASH = function() {
        var swf;
        if (document.all) try {
            if (swf = new ActiveXObject("ShockwaveFlash.ShockwaveFlash")) return Version.FLASH_VERSION = parseInt(swf.GetVariable("$version").split(" ")[1].split(",")[0]),
            !0
        } catch(e) {
            return ! 1
        } else try {
    ...
    Version.canUseSwf = Version.ABLE_FLASH && Version.FLASH_VERSION >= 19,
    Version
}),
qcVideo("uploader",
function(Log, SHA, constants, validate, Code, util, Reporter, ErrorCode, Version, AS3_Worker) {
    window.MXI_DEBUG = !1,
    window.__monitor = util.monitor;
    var uploader, SDKVersion = "20170502",
    ...
    dispatch = function(args) {
        for (var i = 0,
        j = listeners.length; j > i; i++) listeners[i](args)
    },
    getReport = function(file) {
        return file._report
    },
    ...
    _FilesAdded = function(up, files) {
        ...
        plupload.each(files,
        function(file) {
            if (!file.size) return dispatch({
                code: Code.CANNOT_GET_FILESIZE,
                messageTC: "无法获取视频文件大小",
                solution: up.settings.multi_selection ? "可尝试设置初始化方法中传入的参数 disable_multi_selection 为 true": ""
            }),
            void __monitor(up.settings.multi_selection ? 2605790 : 2605791);
            ...
            -1 == $.inArray(file.type, FinalFileTypes) ? (deleteIds.push(file.id), illegalType = !0) : (validate.checkFileName(file.filename) || (Log.log(file.name, file.filename), file.filename = "请重命名！"), file.status = plupload.HASHING, file._report = Reporter({
                name: file.filename,
                size: file.size,
                ...
                file: file
            }), SHA.add(file), chunk_seqs[file.id] = 0, __monitor(2604994))
        }),
        $.each(deleteIds,
        function(_, id) {
        ...
        illegalType && dispatch({
            ...
            messageTC: "只能上传视频文件，已自动过滤掉非视频文件",
            solution: ""
        })
    },
    ...
    _BeforeUpload = function(up, file) {
        file.usedTime = void 0 == file.usedTime ? 0 : file.usedTime,
    ...
    _StateChanged = function(up) { (up.state == plupload.QUEUED || up.state == plupload.STARTED) && dispatch({
        ...
        if (res) {
            if (res = $.parseJSON(res), 0 == res.code) {
                if (clearInterval(file.speedTimer), 1 == res.flag) {
                    ...
                    reportParam.chunk_flag = ++chunk_seqs[file.id],
                ...
                file.speedInKB = parseInt(file.realFlux / file.usedTime) || 0,
                file.speed = util.compute_speed(file.realFlux / file.usedTime * 1e3),
                file.loaded != file.size ? (file_retry_time[file.id] = 0, file.speedTimer = setInterval(function() {
                    ...
                    100 != virtualPercent && (virtualPercent = virtualPercent.toFixed(2)),
                    file.virtualPercent = virtualPercent > 0 ? virtualPercent: 0,
                    up.trigger("UploadProgress", file)
                },
                1e3 / 3)) : file.virtualPercent = 100,
    ...
    _Error = function(up, err) {
        ...
        if (Log.error("code", code), code == plupload.FILE_SIZE_ERROR) dispatch({
            ...
            messageTC: "文件大小超出限制，" + err.file.name + "(" + util.getHStorage(err.file.size) + ")已被自动过滤",
            file: err.file
        });
            ...
            file_retry_time.hasOwnProperty(fileId) || (file_retry_time[fileId] = 0),
            file_retry_time[fileId] > MAX_RETRY_TIME ? dispatch({
                code: Code.UPLOAD_FAIL,
                messageTC: "文件上传失败",
                file: file,
                errorCode: code
    ...
    return Log.log("uploadsdk", SDKVersion),
    {
        version: SDKVersion,
        eachFiles: function(fn) {
        ...
        reUpload: function() {
            eachFiles(function(file) {
                file.status == plupload.FAILED && (file.status = plupload.QUEUED, file.loaded = 0, file_retry_time[file.id] = 0),
                file.is_from_resume = !0
            }),
            uploader.start(),
            dispatch({
                code: Code.UPLOADER_STATUS_CHANGE,
        ...
        deleteFile: function(fid) {
            var file = this.getFile(fid);
            file.speedTimer && (clearInterval(file.speedTimer), file.speedTimer = null),
            SHA.del(fid),
        ...
        init: function(setting, handlers) {
            ...
            File.prototype.slice || (File.prototype.webkitSlice ? File.prototype.slice = File.prototype.webkitSlice: File.prototype.mozSlice && (File.prototype.slice = File.prototype.mozSlice)),
            Version.canUseSwf = !setting.forceH5Worker && Version.canUseSwf,
            Version.canUseSwf && AS3_Worker.loadSwf(),
            __monitor(Version.canUseSwf ? 2604997 : 2604998),
            Log.log("Version.FLASH_VERSION", Version.FLASH_VERSION, setting.forceH5Worker, Version.canUseSwf),
            setting.isTranscode || delete setting.transcodeNotifyUrl,
            setting.secretKey || setting.getSignature || alert("您还没配置上传签名的获取获取方式，请参考api文档说明，补充签名串"),
            setting.sha1js_path && SHA.setWorkPath(setting.sha1js_path),
            SHA.addListener(function(file, percent, info) {
                return info && info.code ? (file.status = Code.SHA_FAILED, dispatch({
                    code: Code.SHA_FAILED,
                    messageTC: "sha error " + info.code,
                    file: file
                }), void __monitor(2604996)) : void(100 == percent && info && info.sha ? (file.status = plupload.QUEUED, file.hashed_size = info.sha, dispatch({
                    code: Code.UPLOAD_WAIT,
                    messageTC: "文件等待上传",
                    cost_time: info.cost,
                    file: file
                }), setting.after_sha_start_upload && self.startUpload(), __monitor(2604995)) : percent >= 0 && 100 >= percent && dispatch({
            ...
            isMQQ = /QQ\/\d+/i.test(navigator.userAgent),
            disableMultiSelection = "iOS" === mOxie.Env.os && (isWX || isMQQ),
            ...
            mimeTypes && mimeTypes.length > 0 ? mimeTypes.forEach(function(e, i) {
                e = e.toUpperCase(),
                FinalFileTypes.indexOf(e) > -1 && exts.push(e)
            }) : exts = FinalFileTypes;
            var isV3 = setting.web_upload_url.indexOf("/v3/") > -1; (uploader = new plupload.Uploader({
                ...
                multipart: !1,
                chunk_size: isV3 ? "1mb": "5mb",
                chunk_size_part1: isV3 ? "1mb": "512kb",
                chunk_size_step: isV3 ? "1mb": "2mb",
                multipart_params: multipart_params,
                filters: {
                    max_file_size: setting.filters && setting.filters.max_file_size || "20gb",
                    mime_types: "Windows" == mOxie.Env.OS && "Chrome" != mOxie.Env.browser ? [{
                        title: "media files",
                ...
                expiredAfter: setting.expiredAfter || 216e5,
                signature: function(argStr, cb) {
                    if (setting.secretKey) if (isV3) {
                        if ("string" != typeof argStr) {
                            var uin = document.cookie.match(/ownerUin=(\d+);/);
                            uin = uin && uin[1] || 0,
                            argStr.uid = uin;
                            var s = [];
                            for (var a in argStr) s.push(a + "=" + argStr[a]);
                            argStr = s.join("&")
                        }
                        var sha = CryptoJS.HmacSHA1(argStr, setting.secretKey);
                        sha.concat(CryptoJS.enc.Utf8.parse(argStr)),
                        cb(CryptoJS.enc.Base64.stringify(sha))
                    } else cb(CryptoJS.enc.Base64.stringify(CryptoJS.HmacSHA1(argStr, setting.secretKey)));
                    else setting.getSignature ? setting.getSignature(argStr,
                    function(signature) {
                        cb(signature)
                    }) : alert("无签名串，请参考api文档说明，补充签名串")
                }
            })).init(),
            isV3 ? __monitor(2605710) : __monitor(2605709);
            ...
            self.addListener(function(args) {
                ...
                case Code.UPLOAD_SHA:
                    ...
                    break;
                case Code.SHA_FAILED:
                    updateFile(getUpdateFileArgs(args.code, args.file, 0)),
                    Log.error(args.file.id, args.messageTC);
                    break;
                case Code.UPLOAD_WAIT:
                    updateFile(getUpdateFileArgs(args.code, args.file)),
                    Log.debug(args.file.id, args.messageTC, args.file.name, (args.cost_time / 1e3).toFixed(2));
                    break;
                case Code.WILL_UPLOAD:
                    Log.debug(args.file.id, args.messageTC),
                    updateFile(getUpdateFileArgs(args.code, args.file));
                    break;
            ...
            self.addListener(function(args) { - 1 === $.inArray(args.code, constants.HIGH_FREQUENCY_STATUS) && _dispatchStatus()
            });
            var filterError = handlers.onFilterError || empty;
            return self.addListener(function(args) { (args.code == Code.ILLEGAL_TYPE || args.code == Code.ILLEGAL_NAME || args.code == Code.OVER_MAX_SIZE || args.code == Code.CANNOT_GET_FILESIZE) && filterError(args)
            }),
            uploader
        },
        initUGC: function(setting, handlers) {
            var originalFn = setting.getSignature;
            return originalFn || alert("需要定义回调函数去远程服务器获取签名"),
            $.extend(setting, {
                web_upload_url: "https://vod2.qcloud.com/v3/index.php",
                forceH5Worker: !0,
                getSignature: function(argObj, done) {
                    originalFn && originalFn(argObj, done)
                }
            }),
            qcVideo.uploader.init(setting, handlers)
        }
    }
}),
